...
const DURATION_UNIT = 86400
const MAX_DURATION = DURATION_UNIT * 365 * 3;
const MIN_AMOUNT = ethers.utils.parseEther("1")

const assertNotExist = async (system: SystemStaking3, tokenId: BigNumberish) => {
    ...
    describe("stake flow", () => {
        ...
        describe("normal withdraw", () => {
            ...
            it("invalid token id", async () => {
                ...
                [
                    system.connect(staker).timeToUnstake(invalidTokenId),
                    system.connect(staker).timeToWithdraw(invalidTokenId),
            ...
            it("check duration of locked", async () => {
                expect(
                    await system.connect(alice).timeToUnstake(tokenId)
            ...
            describe("unlock", () => {
                ...
                describe("unstake", () => {
                    ...
                    describe("withdraw", () => {
                        beforeEach(async () => {
                            expect(await system.timeToWithdraw(tokenId)).to.be.equal(3 * DURATION_UNIT)
                            await advanceBy(BigNumber.from(3 * DURATION_UNIT))
                            expect(await system.timeToWithdraw(tokenId)).to.be.equal(0)
                        })
                        it("not owner", async () => {
                            await expect(
        ...
        describe("batch", () => {
            ...
            describe("unlock", () => {
                ...
                it("success", async () => {
                    ...
                    for (let i = 0; i < tokenIds.length; i++) {
                        ...
                        expect(bucket.unlockedAt).to.equal((await ethers.provider.getBlock()).timestamp)
                    }
                })
            })
            ...
            describe("unstake", () => {
                ...
                it("success", async () => {
                    ...
                    for (let i = 0; i < tokenIds.length; i++) {
                        const bucket = await system.bucketOf(tokenIds[i])
                        expect(bucket.unstakedAt).to.equal((await ethers.provider.getBlock()).timestamp)
                    }
                })
            })
        ...
        describe("extend duration", () => {
            ...
            it("time to unstake", async () => {
                await system.connect(staker)["unlock(uint256)"](tokenId)
                await advanceBy(BigNumber.from(DURATION_UNIT))
                expect(await system.connect(staker).timeToUnstake(tokenId)).to.equal(0)
                ...
                await system.connect(staker)["unlock(uint256)"](tokenId)
                expect(await system.connect(staker).timeToUnstake(tokenId)).to.equal(DURATION_UNIT * 2)
            })
        })
